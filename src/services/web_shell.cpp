...
String webShellFoot() {
  ...
  return String("<div class='footer'>Not affiliated with Spoolman or FilaMan "
                "- Open Source Project</div></body></html>");
}

String webShellNav(const char *active) {
  static const char *PATHS[]  = { "/", "/ota", "/logs", "/drying", "/config", "/filaman", "/theme" };
  static const char *LABELS[] = { "Status", "Firmware", "Logs", "Drying", "Limits", "FilaMan", "Theme" };
  ...
  String n = "<style>.nav{display:flex;flex-wrap:wrap;gap:8px;width:100%;max-width:480px;"
             ...
             "<div class='nav'>";
  for (int i = 0; i < 7; i++) {
    // Not applicable on Spoolman, and a tab that only ever says "switched off"
    // is worse than no tab.
    if (!strcmp(PATHS[i], "/filaman") && !backendIsFilaMan()) continue;
    n += String("<a class='") + (strcmp(PATHS[i], active) == 0 ? "on" : "") +
         "' href='" + PATHS[i] + "'>" + LABELS[i] + "</a>";
  }
  n += "</div>";
  return n;
}
