...
void startOtaServer() {
  ...
  ota_server.on("/", HTTP_GET, []() {
    ...
    String html =
      ...
      "reporting weights. The <b>API key</b> is used for everything else, and it "
      "carries the permissions of the user who created it. Creating it from a "
      "dedicated account with a limited role is therefore safer than using the "
      "admin account. Note that a role only takes effect once it is assigned to "
      "that user and the key is created while signed in as them. "
      ...
      "border-left:2px solid #28d49a;border-radius:4px;padding:8px 10px;margin-bottom:14px'>"
      "Permissions SpoolmanScale actually uses:<br>"
      "<b>Read:</b> spools:read, spool_events:read, locations:read, filaments:read, "
      "manufacturers:read<br>"
      "<b>Write:</b> spools:update, spools:create, spools:archive, spools:move_location, "
      "filaments:update, manufacturers:update<br>"
      "Nothing is ever deleted, and no admin permission is needed. "
      "Weight reports go through the device token, not the API key.</div>"
