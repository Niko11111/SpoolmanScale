...
static void serverEnsureStopped() {
  ...
  static String maintPage(int sec) {
    ...
    if (sec == 2) html +=       "<div class='card'>"
      ...
      "<script>"
      "function setLocL(){""var v=parseInt(document.getElementById('locl-in').value);""if(v<5)v=5;if(v>100)v=100;""fetch('/api/loclimit',{method:'POST',body:String(v)})"".then(r=>r.json()).then(d=>{""document.getElementById('locl-s').textContent='Saved: '+d.limit;""setTimeout(()=>{document.getElementById('locl-s').textContent='';},3000);""});}""function setLL(){"
...
static void registerRoutes() {
  ...
  // The status landing page and the theme editor own their own routes.
  // Registered here so everything lands in the one-shot registration.
  registerHomeRoutes(ota_server);
  registerThemeRoutes(ota_server);
