...
const char* const STRINGS[][2] = {
  ...
  { "In FilaMan sind das zwei getrennte Schritte.<br>1. Auf das <b>Zahnrad</b> neben dem Benutzernamen klicken, <b>API Keys</b> öffnen und einen Key anlegen, z.B. mit dem Namen <i>Spoolman Scale</i>. Er wird nur einmal angezeigt, also gleich oben ins Feld API-Key kopieren.<br>2. <b>Admin-Bereich &gt; Geräte</b> öffnen und <b>Gerät erstellen</b> klicken, z.B. ebenfalls <i>Spoolman Scale</i>. Den sechsstelligen Code, der dann erscheint (auch nur einmal), oben ins Feld Gerätecode eintragen und registrieren. Dafür sind Admin-Rechte nötig.",
    "FilaMan needs two separate steps.<br>1. Click the <b>gear icon</b> next to your user name, open <b>API Keys</b> and create a key, e.g. named <i>Spoolman Scale</i>. It is shown only once, so copy it straight into the API key field above.<br>2. Open <b>Admin Panel &gt; Devices</b> and click <b>Create Device</b>, e.g. named <i>Spoolman Scale</i>. Enter the six character code it shows (also only once) in the device code field above and press Register. This step needs admin rights." },  // STR_W_FM_SETUP
