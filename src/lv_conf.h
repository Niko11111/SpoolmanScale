...
/* Must stay 1. With a manual lv_tick_inc() the tick only advances while the
 * main loop runs, so the ~37 lv_timer_handler() calls that sit inside long
 * blocking operations (WiFi connect, NTP, OTA download, backend queries) never
 * reached LV_DISP_DEF_REFR_PERIOD and drew nothing - the display froze exactly
 * when it was supposed to show progress. */
...
#define LV_FONT_CUSTOM_DECLARE \
    LV_FONT_DECLARE(lv_font_montserrat_ext_8)   \
    LV_FONT_DECLARE(lv_font_montserrat_ext_10)  \
    ...
    LV_FONT_DECLARE(lv_font_montserrat_ext_26)  \
